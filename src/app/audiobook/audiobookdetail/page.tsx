...
export default function AudiobookDetail() {
  ...
  const [isMarked, setIsMarked] = useState(false); // Khởi tạo state với giá trị false
  ...
  const handleMarkBook = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.88:8386/nexia-service/v1/secure/mark?contentId=${id}&action=${action}&type=1`, {
        method: 'POST',
        headers: {
  ...
  const handleShare = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.88:8386/nexia-service/v1/common/share?contentId=${id}&type=1`, {
        method: 'POST',
        headers: {
          'userId': '1',
          'Client-Type': 'Web',
  ...
  const handleLikeBook = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.88:8386/nexia-service/v1/secure/like?contentId=${id}&action=${action}&type=1`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
      ...
      if (data && data.code === 200) {
        setIsLiked(!isLiked); // Đảo ngược trạng thái like
        console.log(isLiked ? 'Đã bỏ like' : 'Đã like:', data.data);
      } else {
        console.error('Lỗi khi like:', data?.message || 'Không có phản hồi từ server');
      }
    } catch (error) {
      console.error('Lỗi khi gọi API like:', error instanceof Error ? error.message : 'Lỗi không xác định');
    }
  };
      ...
      <div className="px-4 pb-4">
        ...
        <div className="flex w-full border-b border-gray-800">
          <button
            ...
            className={`flex-1 px-2 sm:px-4 py-2 text-sm sm:text-base whitespace-nowrap ${activeTab === 'gioithieu'
